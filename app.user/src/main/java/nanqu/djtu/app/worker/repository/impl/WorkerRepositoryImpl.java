...
//    }
    ...
    public int selectallDoMaintenanceSum(String userId) {
        StringBuilder builder = new StringBuilder();
        builder.append("SELECT count(MA.listNumber) FROM baoxiu_maintenancelist AS MA  LEFT JOIN baoxiu_workerinfo AS INF ON(MA.repairGroupId = INF.repairGroupId) WHERE MA.listState < 3 AND INF.userId = ? ");
